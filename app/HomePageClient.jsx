...
// --- Componente do Card de Anúncio (Polido) ---
const AnuncioCard = ({ anuncio, type, compareList, onCompareChange }) => {
  const isMaquina = type === 'maquinas';
  // Usamos a thumbnail (definida no seed) ou um fallback
  const imageUrl = anuncio.imagens && anuncio.imagens.length > 0 
    ? anuncio.imagens[0].thumbnailUrl 
    : `https://placehold.co/400x300/e2e8f0/333?text=Sem+Foto`;
      ...
      <div className="relative">
        ...
        <Link href={linkHref}>
          <div className="w-full h-48 bg-gray-200 relative">
            {/* --- POLIMENTO 1: Troca de <img> por <Image /> --- */}
            {/* Isso garante lazy loading, otimização (WebP) e previne CLS. */}
            <Image 
              src={imageUrl} 
              alt={anuncio.titulo || anuncio.nome} 
              fill // 'fill' faz a imagem cobrir o contêiner (requer 'relative' no pai)
...
// --- POLIMENTO 2: Paginador "Inteligente" ---

// Função helper para criar a lógica de "..."
const generatePagination = (current, total) => {
  if (total <= 7) {
    return Array.from({ length: total }, (_, i) => i + 1); // [1, 2, 3, 4, 5, 6, 7]
  }
...
const Paginator = ({ currentPage, totalPages }) => {
  ...
  const pages = generatePagination(currentPage, totalPages); // Usa a nova função

  const createPageUrl = (page) => {
    const params = new URLSearchParams(searchParams);
    params.set('page', page.toString());
    return `/?${params.toString()}`; // Assume que está na raiz (ou ajuste o path)
  };
      ...
      {pages.map((page, index) => 
        page === '...' ? (
          ...
          <Link 
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium ${currentPage === page ? 'bg-yellow-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'}`}
          >
            {page}
          </Link>
        )
...
// --- Componente Principal do Cliente (Polido) ---
    ...
    setCompareList(currentList => {
      if (isChecked) {
        if (currentList.length < 3) {
          ...
          setShowMessage(true);
          setTimeout(() => setShowMessage(false), 3000); // O "toast" some após 3s
          return currentList;
        }
      } else {
      ...
      <AnimatePresence>
        {showMessage && (
          <motion.div
            ...
            className="fixed bottom-24 left-1/2 -translate-x-1/2 bg-red-500 text-white font-semibold py-2 px-4 rounded-lg shadow-md z-50"
          >
            ...
            <div className="flex items-center gap-4 overflow-x-auto">
              ...
              {compareList.map(item => (
                <div key={item.id} className="flex items-center gap-2 flex-shrink-0">
                  {/* --- POLIMENTO 1: Troca de <img> por <Image /> --- */}
                  <Image 
                    src={item.imagens[0]?.thumbnailUrl} 
                    alt={item.nome} 
                    width={40} 
                    height={40} 
